...
import { RunFixture } from './shared/fixtures'
...
describe('PlasmaswapRouter', () => {
  ...
  beforeEach(async function() {
    const fixture = await loadFixture(RunFixture)
    token0 = fixture.token0
    token1 = fixture.token1
    router = fixture.routerSwap
  })
  ...
  it('getAmountOut', async () => {
    ...
    await expect(router.getAmountOut(bigNumberify(0), bigNumberify(100), bigNumberify(100))).to.be.revertedWith(
      'PlasmaswapLibrary: INSUFFICIENT_INPUT_AMOUNT'
    )
  ...
  it('getAmountIn', async () => {
    ...
    await expect(router.getAmountIn(bigNumberify(0), bigNumberify(100), bigNumberify(100))).to.be.revertedWith(
      'PlasmaswapLibrary: INSUFFICIENT_OUTPUT_AMOUNT'
    )
  ...
  it('getAmountsOut', async () => {
    ...
    await expect(router.getAmountsOut(bigNumberify(2), [token0.address])).to.be.revertedWith(
      'PlasmaswapLibrary: INVALID_PATH'
    )
...
describe('fee-on-transfer tokens', () => {
  ...
  beforeEach(async function() {
    ...
    await fixture.factorySwap.createPair(DTT.address, WETH.address)
    const pairAddress = await fixture.factorySwap.getPair(DTT.address, WETH.address)
    pair = new Contract(pairAddress, JSON.stringify(IPlasmaswapPair.abi), provider).connect(wallet)
  })
...
describe('fee-on-transfer tokens: reloaded', () => {
  ...
  beforeEach(async function() {
    ...
    // make a DTT<>WETH pair
    await fixture.factorySwap.createPair(DTT.address, DTT2.address)
    const pairAddress = await fixture.factorySwap.getPair(DTT.address, DTT2.address)
  })
